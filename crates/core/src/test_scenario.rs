    ...
    P: PlanConfig<String> + Templater<String> + Send + Sync,
{
    ...
    /// The agent spec used to build the agent store
...
pub mod tests {
    ...
    use crate::generator::agent_pools::AgentSpec;
    ...
    async fn gas_limit_override_works() {
        let anvil = spawn_anvil();
        let scenario = get_test_scenario(&anvil, None, None).await.unwrap();
        let (spam_txs, _nonces) = scenario
            .load_txs(PlanType::Spam(20, |tx| {
    ...
    async fn rpc_batch_size_controls_num_rpc_batches_sent() -> Result<()> {
        ...
        let mut scenario_batched = get_test_scenario(&anvil, None, None).await?;
    ...
    async fn new_scenario_rejects_mismatched_chain_ids() {
        ...
        let scenario = get_test_scenario(&anvil, Some(&anvil2), None).await;
    ...
    async fn gas_price_override_skips_gas_price_adder() -> Result<()> {
        ...
        let mut scenario = get_test_scenario(&anvil, None, Some(override_gas_price)).await?;
